...
export async function getGradingBasisOptions(input: z.infer<typeof GetGradingBasisOptionsInput>) {
  ...
  const response = await workdayFetch(
    `/wday/sirg/protectedapi/studentRegistration/v1/ubc/values/savedSchedule/gradingBasis${query}`
  );
  return response;
}
