...
export async function getCourseSections(input: z.infer<typeof GetCourseSectionsInput>) {
  ...
  if (input.academicLevelId) params.set("academicLevel", input.academicLevelId);
  if (input.courseId) params.set("course", input.courseId);
  if (input.view) params.set("view", input.view);
  if (input.facets !== undefined) params.set("facets", String(input.facets));
...
export async function getCourseSection(input: z.infer<typeof GetCourseSectionInput>) {
  const params = new URLSearchParams();
  if (input.view) params.set("view", input.view);
