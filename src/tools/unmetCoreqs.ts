...
export async function getUnmetCoreqs(
  input: z.infer<typeof GetUnmetCoreqsInput>
) {
  ...
  const response = await workdayFetch(
    `/wday/sirg/protectedapi/studentRegistration/v1/ubc/values/savedSchedule/unmetCoreqs${query}`
  );
  return response;
}
