...
describe('Widget parser', () => {
  ...
  it('parses redacted widget tree with multiple sections', () => {
    const result = (client as any).parseWidgetTree(REDACTED_WIDGET_RESPONSE);
  ...
  it('parses minimal flat array response', () => {
    const result = (client as any).parseWidgetTree(REDACTED_MINIMAL_RESPONSE);
  ...
  it('handles empty response', () => {
    const result = (client as any).parseWidgetTree(REDACTED_EMPTY_RESPONSE);
  ...
  it('handles nested data structure', () => {
    const result = (client as any).parseWidgetTree(REDACTED_NESTED_RESPONSE);
    expect(result.sections).toHaveLength(1);
  ...
  it('skips widgets without displayName', () => {
    const result = (client as any).parseWidgetTree(REDACTED_NO_DISPLAY_NAME);
    expect(result.sections).toHaveLength(0);
    expect(result.debug.totalWidgets).toBe(1);
    expect(result.debug.parsedWidgets).toBe(0);
    expect(result.debug.skippedWidgets).toBe(1);
