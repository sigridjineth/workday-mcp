...
export async function validateSavedScheduleCreate(input) {
    const response = await workdayFetch("/wday/sirg/protectedapi/studentRegistration/v1/ubc/savedSchedules", {
        method: "POST",
        headers: {
...
export async function createSavedSchedule(input) {
...
export async function getSavedSchedule(input) {
    const params = new URLSearchParams(input.params || {});
    const query = params.toString() ? `?${params.toString()}` : "";
    const response = await workdayFetch(`/wday/sirg/protectedapi/studentRegistration/v1/ubc/savedSchedules/${input.scheduleWid}${query}`);
    return response;
}
...
export async function validateSavedScheduleUpdate(input) {
    const response = await workdayFetch(`/wday/sirg/protectedapi/studentRegistration/v1/ubc/savedSchedules/${input.scheduleWid}`, {
        method: "PUT",
        headers: {
            "Content-Type": "application/json",
            "x-validate-only": "1",
...
export async function updateSavedSchedule(input) {
    if (input.confirmationToken !== "CONFIRMED") {
        throw new Error("Confirmation required. Set confirmationToken to 'CONFIRMED' after reviewing payload.");
    }
    const response = await workdayFetch(`/wday/sirg/protectedapi/studentRegistration/v1/ubc/savedSchedules/${input.scheduleWid}`, {
...
//# sourceMappingURL=savedSchedule.js.map