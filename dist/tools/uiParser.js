...
export async function getCourseSectionUiDetail(input) {
...
export async function searchCourseSectionsUiFacets(input) {
    const cfg = getWorkdayConfig();
    ...
    if (input.academicLevelId)
        body.set("academicLevel", input.academicLevelId);
    if (input.deliveryModeId)
        body.set("deliveryMode", input.deliveryModeId);
    if (input.courseSubjectId)
        body.set("courseSubject", input.courseSubjectId);
    if (input.credits)
        body.set("credits", input.credits);
    if (input.pageSize)
        body.set("pageSize", String(input.pageSize));
    const cookieParts = [
        `wd-browser-id=${cfg.wdBrowserId}`,
        `JSESSIONID=${cfg.jsessionId}`,
        `UserSignedIn=1`,
...
export async function getRegistrationTroubleshoot(input) {
    const cfg = getWorkdayConfig();
    let path;
